...
| A constraint bounds the **result** of a write, not just its start | NetBox's post-save `_validate_objects()`, restored and called from inside the compensated region; `_require_rotate` on both source and destination | `test_policy_gate.ConstrainedPermissionTest` — 5 tests |
| A bulk update cannot strand an OpenBao write | material-bearing bulk updates are refused outright — the batch shares one transaction the compensator cannot reach | `test_policy_gate.BulkMaterialWriteTest` — 3 tests |
...
| REST quick-add never returns material or unrelated bindings | Write-only request fields, a fixed response serializer, target/service-scoped assignment query, and `no-store` | `test_api.QuickAddSSHAPITest` |
| Server-generated SSH private keys go directly to OpenBao | Write-only generation controls consumed inside `perform_create()` and the existing material transaction | `test_api.CredentialWriteTest.test_create_can_generate_an_ssh_key_without_returning_private_material`, `test_security.test_server_side_generation_inputs_are_write_only` |
